...
|---|---|---|
| 1 — Manifest Validity | GEC Manifest is current and unrevoked (KIA-07 §5.4 staleness window) | ALE-059, failed_check: 1 |
| 2 — Policy Currency | `cedar_policy_hash` computed fresh over the active Cedar Policy Set — not cached | ALE-059, failed_check: 2 |
| 3 — KIA Attestation | Kernel's KIA signing key not in Revocation Registry (KIA-07 §8) | ALE-059, failed_check: 3 |
| 4 — Revocation Status | Agent XPID not revoked (Revocation Registry or CAEP signal per KIA-07 §6.5) | ALE-059, failed_check: 4 |
...
**Compliance handshake volumetric abuse:** A malicious or compromised resource provider identity can issue high-frequency requests to `/.well-known/soos-acd`, forcing the GEC to repeatedly run the four-check sequence and the KIA-signing operation — both non-trivial cost relative to the request itself, since nothing in the protocol requires a prior relationship before a handshake. Defense: the GEC MUST enforce a per-`resource_provider_id` rate limit (deployment-defined), SHOULD apply a stricter limit to identities with no prior successful handshake on record, and MAY reject repeated requests within the session-caching window outright when a valid cached record already exists. Residual risk: a distributed attack using many distinct resource_provider_id values isn't fully addressed by per-identity limiting alone; deployments facing that threat model should add network-level volumetric defenses.

**Formal analysis status:** ACD-03 has not undergone formal security analysis. The KIA-signature and CSPRNG-based UUID generation requirements follow established cryptographic practice. Formal analysis is planned post-Vienna.

---

## SOOS stack context

ACD sits at Layer 5 — Resource Access Governance. It depends on KIA (draft-sato-soos-kia-07) for kernel identity and signing, CAP (draft-sato-soos-cap-06) and CAP-RRS (draft-sato-soos-cap-rrs-04) for the CAP Profiles it references, AEP (draft-sato-soos-aep-04) for the session lifecycle within which it operates, MJWT (draft-sato-soos-mjwt-06) for the mandate JWT it binds, GAR (draft-sato-soos-gar-08) for audit logging, HEM (draft-sato-soos-hem-07) for escalation on validation failure, and RGP (draft-sato-soos-rgp-02) for the `acd_required` trigger. It is consumed by resource providers implementing the ACD compliance handshake. Related drafts: [KIA](/drafts/kia) · [CAP](/drafts/cap) · [GAR](/drafts/gar) · [AEP](/drafts/aep) · [MJWT](/drafts/mjwt) · [RGP](/drafts/rgp) · [KEE-1](/drafts/kee)
